...
double  convert(std::string  price)
{
    std::istringstream  in(price);
    double   vcl;
    in >> vcl;
    return vcl;
}
...
void  BitcoinExchange::Read_csv_file()
{
    ...
    while(!std::getline(file,buffer))
    {
        ...
        double  value =  convert(data);
        key_value[date] = value; 
    }
    file.close();
}
...
double  valid_value(std::string  value)
{
 ...
 void     BitcoinExchange::exchange_with_price(std::string  date,double value)
 {
    ...
    if(key_value.find(date) != key_vlaue.end())
        std::cout  << date << " => " << value << " = " << value * it->second <<  "\n";
    else
    {
         std::map<std::string ,double>::iterator it2 = key_value.end();
         if(it2 == it)
            return;
        it--;
        std::cout  << date << " => " << value << " = " << value * it->second <<  "\n";
 ...
 void  BitcoinExchange::execute(const char *File) const
 {
    Read_csv_file();
    std::string  buff;
    ...
    while(!getline(file,buff))
    {
        ...
        if(vl == -1)
            continue;
        exchange_with_price(data.value);
    }
